...
$(function(){
	...
	$(".cancel").click(function(){
		...
		if(text){
			location.replace("/mars/member/detail?memberNo="+${boardAdDto.memberNo})
		}
	});
...
<form action="edit" method="post" class="form-submit">
	<input type="hidden" name="boardAdNo" value="${boardAdDto.boardAdNo}">
		    ...
		    <div class="card border-primary mb-3" style="height: 600px;">
		      <div class="card-header">
		        <div class="row">
		          ...
		          <div class="col-md-8">
		            <h4 class="text-primary text-center" style="margin-top: 1%;">광고 게시물 수정하기</h4>
		          </div>
		          <div class="col-md-2">
		      		...
		      		<div class="col-md-7">
		      			...
		      			<div id="carouselExampleIndicators" class="carousel slide" data-bs-interval="false">
								  <div class="carousel-indicators">
								  	<c:forEach items="${boardAdAttachVO}" var="boardAdAttachVO" varStatus="status">
								  ...
								  <div class="carousel-inner" >
								  	<c:forEach items="${boardAdAttachVO}" var="boardAdAttachVO" varStatus="status">
									  	<div class="carousel-item" v-bind:class="{'active':${status.index == 0}}">
									  		<img src="${pageContext.request.contextPath}/file/download/${boardAdAttachVO.attachNo}" class="d-block w-100" style="height: 480px;">
										 </div>
									 </c:forEach>
								 </div>
		      		...
		      		<div class="col-md-5">
					    	<div class="row">
					    		<div class="col-md-2 right">
						    		<img src="${pageContext.request.contextPath}/file/download/${boardAdMemberVO.attachNo}" class="img img-circle">		    		
					    		</div>
					    		<div class="col-md-10 left bottom">
					    			<span class="nickname">${boardAdMemberVO.memberNick}</span>
					    		</div>
					    	</div>
					    	...
					    	<div class="row">
					    		<textarea id="contentCheck" class="form-control content" rows="6" name="boardAdContent" placeholder="문구를 입력하세요">${boardAdDto.boardAdContent}</textarea>
					    		<div class="right">
										<span class="length">
											<span class="count">${boardAdDto.boardAdContent.length()}</span>
											/
					    	...
					    	<div class="row mt-4">
					    		<select name="adNo" class="form-select">  
					    			<c:forEach var="adList" items="${adList}">
					    				<option value="${adList.adNo}" <c:if test="${adList.adNo == boardAdDto.adNo}">selected</c:if> >노출 횟수 ${adList.adCount}번 : 가격 ${adList.adPrice}원</option>
					    			</c:forEach>
					    		</select>
					    	</div>
					    	...
					    	<!-- 광고 링크 입력 -->
					    	<div class="row mt-4">
					    		<input type="text" name="boardAdLink" class="form-control" placeholder="광고 링크" autocomplete="off" value="${boardAdDto.boardAdLink}">
					    	</div>
					    	...
					    	<div class="row mt-4 form-check form-switch" style="display: flex;">
										...
										<div class="col-md-3">
											<input type="checkbox" name="boardAdIsReply" value="1" class="form-check-input fs-5" style="margin-left: 0;" id="replyCheck" <c:if test="${boardAdDto.boardAdIsReply == 1}">checked</c:if>>
										</div>
